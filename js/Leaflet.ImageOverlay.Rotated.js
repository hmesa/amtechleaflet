...
L.ImageOverlay.Rotated = L.ImageOverlay.extend({
    ...
    initialize : function(image, topleft, topright, bottomleft, options) {
        ...
        if (typeof (image) === 'string') {
            ...
            // Assume that the first parameter is an instance of HTMLImage or
            // HTMLCanvas
    ...
    _initImage : function() {
        ...
        if (this._url) {
            ...
            img.style.display = 'none'; // Hide while the first transform (zero
            // or one frames) is being done
        ...
        var div = this._image = L.DomUtil.create('div', 'leaflet-image-layer '
    ...
    _reset : function() {
        ...
        this._bounds = L.latLngBounds(this._map
                .layerPointToLatLng(pxBounds.min), this._map
                .layerPointToLatLng(pxBounds.max));
        ...
        var scaleY = pxTopLeft.distanceTo(pxBottomLeft) / imgH
                * Math.cos(skewY);

        this._rawImage.style.transformOrigin = '0 0';

        this._rawImage.style.transform = 'translate(' + pxTopLeftInDiv.x
                + 'px, ' + pxTopLeftInDiv.y + 'px)' + 'skew(' + skewY + 'rad, '
                + skewX + 'rad) ' + 'scale(' + scaleX + ', ' + scaleY + ') ';
    },
    ...
    setUrl : function(url) {
        // L.ImageOverlay.prototype.setUrl.call(this,url);
...
/*
 * 🍂factory imageOverlay.rotated(imageUrl:
 * String|HTMLImageElement|HTMLCanvasElement, topleft: LatLng, topright: LatLng,
 * bottomleft: LatLng, options?: ImageOverlay options) Instantiates a
 * rotated/skewed image overlay, given the image URL and the `LatLng`s of three
 * of its corners.
 * 
 * Alternatively to specifying the URL of the image, an existing instance of
 * `HTMLImageElement` or `HTMLCanvasElement` can be used.
 */
L.imageOverlay.rotated = function(imgSrc, topleft, topright, bottomleft,
        options) {
    return new L.ImageOverlay.Rotated(imgSrc, topleft, topright, bottomleft,
            options);
};